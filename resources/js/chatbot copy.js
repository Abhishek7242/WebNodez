        ...
        messageElement.innerHTML = `
			...
			<div class="message-content">
				...
				<div class="warning-message" style="color: #ff4444; font-size: 0.7em; margin-top: 8px;">
					âš ï¸ Please do not reload the page while talking to our team to maintain the connection.
				</div>
			</div>
		`;
...
class Chatbot {
    ...
    addUserMessage(message) {
        ...
        // If user typed their email in conversation, capture it
        const emailMatch = message.match(/(?:my\s+email\s+is|email\s*[:=]\s*)([^\s]+@[^\s]+\.[^\s]+)/i);
        if (emailMatch) {
            this.userEmail = emailMatch[1];
    ...
    async generateResponse(message) {
        ...
        try {
            ...
            const emailProvided = this.userEmail && this.userEmail !== 'userskipped@email.com';
            const emailInstruction = emailProvided
                ? 'Email is already provided by user. Do not ask for email again.'
                : 'Email not provided yet (user may have skipped). Politely ask for their email during conversation when it makes sense. If they prefer not to share, continue helping without blocking.';

            const prompt = `
			You are Harmony, a friendly and professional AI assistant for Linkuss, a technology company.
			Linkuss provides web-development, app-development, UI/UX design, and e-commerce solutions.
			Linkuss has 3 years of experience, 100+ clients, and a 98% success rate. Linkuss has blogs and portfolio on website. We have many projects on website.
			...
			Linkuss is a software development company.
        ...
        termsNotice.innerHTML = `
			...
			<div class="terms-agreement">
				...
				<div class="terms-agreement-buttons">
					<button class="terms-agreement-btn terms-agree-btn">âœ“ Agree</button>
					<button class="terms-agreement-btn terms-disagree-btn">âœ• Disagree</button>
				</div>
			</div>
		`;
    ...
    showWelcomeMessage() {
        ...
        messageElement.innerHTML = `
			...
			<div class="message-content">
				<span class="typing-text">Hello! ðŸ‘‹ I'm Harmony, your Linkuss assistant. How can I help you today?</span>
			</div>
		`;
        messagesContainer.appendChild(messageElement);
        messagesContainer.scrollTop = messagesContainer.scrollHeight;
    ...
    showEmailInput() {
        ...
        emailContainer.innerHTML = `
			<div class="email-input-wrap">
				...
				<div class="email-error hidden">Please enter a valid email address.</div>
			</div>
			<div class="email-buttons-container">
				<button class="email-submit-btn skip-btn">Skip</button>
				<button class="email-submit-btn submit-btn">Submit</button>
			</div>
		`;
        ...
        const submitBtn = emailContainer.querySelector('.submit-btn');
        const skipBtn = emailContainer.querySelector('.skip-btn');
        ...
        submitBtn.addEventListener('click', () => {
            ...
            if (this.validateEmail(email)) {
                ...
                // Show inline error (do not add a chat message)
                if (emailError) {
        ...
        skipBtn.addEventListener('click', () => {
            if (emailError) emailError.classList.add('hidden');
            this.userEmail = 'userskipped@email.com';
            this.hasProvidedEmail = true;
            this.userSkippedEmail = true;

            // Store the skip action as a user message
            const skipMessage = `I prefer to skip providing my email`;
            ...
            // Add skip action as user message
            const userMessageElement = document.createElement('div');
            userMessageElement.className = 'chatbot-message user-message';
